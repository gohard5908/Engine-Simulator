%这部分产生不同cycle时仿真数据（综合退化下），即全部部件均有退化
       ...
       % load EGT_history.txt
        ...
        for i=1:d_i:200   %先测试2组
           ...
           Deter_data.nowlength=length(1:(i-1+d_i)/d_i);
           Deter_data.cycles_out=[0 cycles_out(1:(i-1+d_i)/d_i)];
           Deter_data.TSFC_out=[0 TSFC_out(1:(i-1+d_i)/d_i)'];
           Deter_data.EGT_out=[0 EGT_out(1:(i-1+d_i)/d_i)'];
           Deter_data.Fnet_out=[0 Fnet_out(1:(i-1+d_i)/d_i)'];
           
           savejson('',Deter_data,'D:\Engine_Simulator\Matlab_Simulator_Deterioration\Deter_sim_data\Deter_Sim_Data.json'); 
